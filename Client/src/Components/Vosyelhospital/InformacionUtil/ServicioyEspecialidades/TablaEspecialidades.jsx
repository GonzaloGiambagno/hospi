...
const TablaEspecialidades = () => {
  ...
  const [isSmallScreen, setIsSmallScreen] = useState(false); // Estado para determinar si es una pantalla pequeña
  ...
  const filteredRows = EspecialidadesData.especialidades.filter(
    (especialidad) =>
  ...
  for (let i = 0; i < COLUMN_COUNT; i++) {
    ...
    columns.push(
      <td key={i} className={`px-4 md:p-4 ${isSmallScreen ? 'block' : 'hidden sm:table-cell'} border border-r-2 border-b-0`}> {/* Agrega clases condicionales para ocultar o mostrar la columna en función del tamaño de pantalla */}
        {columnRows.map((especialidad, index) => (
          <Typography
            key={index}
            variant="small"
            color="blue-gray"
